...
void *thread1(void* arg)
{
	...
	char pbase_tmp[security_level + 1];
	...
	for(; tmp_block < tmp_block_max; tmp_block++)
	{
		...
		{
			sprintf(filename, "/mnt/sdb1/data_64m/%ld", tmp_current_id);
		}
		else if(tmp_current_id % 4 == 2)
		{
			sprintf(filename, "/mnt/sdc1/data_64m/%ld", tmp_current_id);
		}
		else if(tmp_current_id % 4 == 3)
		{
			sprintf(filename, "/mnt/sdd1/data_64m/%ld", tmp_current_id);
		}
		else if(tmp_current_id % 4 == 0)
		{
			sprintf(filename, "/mnt/sde1/data_64m/%ld", tmp_current_id);
		}
