...
int main(void)
{
	...
	mpz_init(g_a);
	mpz_init(g_b);
	char tmp_result[security_level + 1];
	char tmp_str[security_level + 1];
	char encrypt_b[(security_level + 1) * level_b];
	char final_result[(security_level + 16) * (block_max / level_b) * 2];
	...
	getPmod();
	void * start_addr = 0;
	int fd = open("database/server", O_RDONLY, S_IRUSR | S_IWUSR);
	...
	for (int i = 0; i < block_max; i++)
	{
		...
		if(i % level_b == 0 && i != 0)
		{
			...
			memcpy(final_result + strlen(final_result), tmp_str, sizeof(tmp_str));
			memset(tmp_result, 0, sizeof(tmp_result));
			memset(tmp_str, 0, sizeof(tmp_str));
			memset(encrypt_b, 0, sizeof(encrypt_b));
		}
	...
	int count = 0;
	int fd1 = open("database/servers_sort", O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRWXO);
