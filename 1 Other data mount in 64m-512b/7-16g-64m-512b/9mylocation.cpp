...
char *mapped_mem1, *mapped_mem2;
long long int flength1, flength2;
long* arr_add;
int final_result[level_b];
int final_count = 0;
typedef struct node
{
	char data[security_level];
	int number;
	struct node* node1;
	struct node* node2;
	struct node* node3;
...
int main(void)
{
	...
	mynode* tree2 = treeGen("database/verifiers_sort");
	//treeVisiting(tree1);
	...
	{
	//printf("123");	std::cin.ignore();
		char tmp_buffer1[security_level + 16];
		char tmp_buffer2[security_level + 16];
		char output_ptr1[64];
		char output_ptr2[64];
		...
		while(final_result[i++] != 0)
		{
			for(int j = 0; j < level_b; j++)
			{
				...
				if(strcmp(tmp_buffer1, tmp_buffer2) != 0)
				{
					...
					fwrite(output_ptr2, sizeof(output_ptr2), 1, fs7);
				}
			}
		}
	}
